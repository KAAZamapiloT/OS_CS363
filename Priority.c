...
// Gantt chart and table printing functions (similar to previous files)
...
static int write_centered(char *buf, int pos, int max, const char *txt, int w) {
    ...
    if (w < 1) return pos;
    if (len >= w) { for (int i = 0; i < w && pos < max; ++i) buf[pos++] = txt[i]; return pos; }
    int left = (w - len) / 2;
...
static void render_gantt(Segment segs[], int segc, int first_time, int last_time) {
    if (segc <= 0 || last_time <= first_time) { printf(" (no timeline)\n"); return; }
    ...
    char line1[4 * MAXLINE]; int p1 = 0;
    memset(line1, 0, sizeof(line1));
    printf("\n==================== GANTT CHART ====================\n");

    for (int i = 0; i < segc; ++i) {
        int dur = segs[i].end - segs[i].start;
        int w = dur * scale; if (w < 3) w = 3;
    ...
    { char buf[32]; int len = snprintf(buf, sizeof(buf), "%d", last_time);
      fputs(buf, stdout); cursor += len; }
    putchar('\n');
    printf("=====================================================\n");
}
...
static void print_table(Proc p[], int n) {
    ...
    printf("\n%-10s %-4s %-4s %-4s %-4s %-10s %-11s %-4s %-11s\n",
    ...
    for (int i = 0; i < n; ++i) {
        ...
        printf("%-10s %-4d %-4d %-4d %-4d %-10d %-11s %-4d %-11s\n",
...
static void priority_nonpreemptive(Proc p[], int n) {
    ...
    long long sumBT = 0;
    Segment segs[MAXSEGS]; int segc = 0;
    for (int i = 0; i < n; ++i) { if (p[i].at < first_arr) first_arr = p[i].at; sumBT += p[i].bt; p[i].done = 0; }
    ...
    while (done < n) {
        ...
        if (idx == -1) {
            int nextAT = INT_MAX;
            for (int i = 0; i < n; ++i) if (!p[i].done && p[i].at > t && p[i].at < nextAT) nextAT = p[i].at;
            push_segment(segs, &segc, "idle", t, nextAT);
            t = nextAT; continue;
        }
        char label[16]; snprintf(label, sizeof(label), "P%d", p[idx].id);
        push_segment(segs, &segc, label, t, t + p[idx].bt);
        t += p[idx].bt;
        p[idx].ct = t; p[idx].done = 1; last_ct = t; done++;
    }
    for (int i = 0; i < n; ++i) { p[i].tat = p[i].ct - p[i].at; p[i].wt = p[i].tat - p[i].bt; }
...
static void priority_preemptive(Proc p[], int n) {
    ...
    long long sumBT = 0;
    Segment segs[MAXSEGS]; int segc = 0;
    for (int i = 0; i < n; ++i) { p[i].rem = p[i].bt; p[i].done = 0; if (p[i].at < first_arr) first_arr = p[i].at; sumBT += p[i].bt; }
    if (t < first_arr) t = first_arr;
    ...
    while (done < n) {
        ...
        if (idx == -1) {
            ...
            int nextAT = INT_MAX;
            for (int i = 0; i < n; ++i) if (p[i].rem > 0 && p[i].at > t && p[i].at < nextAT) nextAT = p[i].at;
            push_segment(segs, &segc, "idle", t, nextAT);
            t = nextAT; seg_start = t; continue;
        }
        if (current != idx) {
            if (current != -1) {
                char lab[16]; snprintf(lab, sizeof(lab), "P%d", p[current].id);
            ...
            current = idx; seg_start = t;
        }
        p[current].rem--; t++;
        if (p[current].rem == 0) {
            p[current].ct = t;
            p[current].done = 1; done++;
            char lab[16]; snprintf(lab, sizeof(lab), "P%d", p[current].id);
            push_segment(segs, &segc, lab, seg_start, t);
            current = -1; seg_start = t;
            if (t > last_ct) last_ct = t;
        }
    }
    for (int i = 0; i < n; ++i) { p[i].tat = p[i].ct - p[i].at; p[i].wt = p[i].tat - p[i].bt; if (p[i].ct > last_ct) last_ct = p[i].ct; }
    render_gantt(segs, segc, first_arr, last_ct);
    print_table(p, n);
    double total_elapsed = (double)(last_ct - first_arr);
    double efficiency = total_elapsed > 0 ? (double)sumBT / total_elapsed * 100.0 : 100.0;
...
int main(void) {
    int n, choice; Proc p[MAXN];
    printf("Select Scheduling Type:\n");
    printf("1. Preemptive Priority\n");
    printf("2. Non-Preemptive Priority\n");
    printf("Choice: ");
    if (scanf("%d", &choice) != 1 || (choice != 1 && choice != 2)) { printf("Invalid choice.\n"); return 0; }
    
    printf("Enter number of processes: ");
    if (scanf("%d", &n) != 1 || n <= 0 || n > MAXN) { printf("Invalid n.\n"); return 0; }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        p[i].ct = p[i].tat = p[i].wt = 0; p[i].rem = p[i].bt; p[i].done = 0;
    }
    
    if (choice == 2) priority_nonpreemptive(p, n);
    else priority_preemptive(p, n);
